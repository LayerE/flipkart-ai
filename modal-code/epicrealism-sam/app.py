...
def download_sd_model():
    ...
    with open("/vol/cache/veryBadImageNegative.pt", "wb") as f:
        f.write(a)
...
image = (
    ...
    .run_function(
        download_models, secrets=[Secret.from_name("HUGGINGFACE_TOKEN")], gpu="any"
    )
    .pip_install("xformers", gpu="any")
    .pip_install("realesrgan", "basicsr>=1.4.2", gpu="any")
    .run_function(
        download_sd_model, secrets=[Secret.from_name("HUGGINGFACE_TOKEN")], gpu="any"
    )
...
class ImageGenerator:
    def __enter__(self):
        ...
        self.pipe.load_textual_inversion(
            cache_path,
            weight_name="veryBadImageNegative.pt",
    ...
    def generate(
        ...
        images_output = []
        negative_prompt = "EasyNegative, ng_deepnegative_v1_75t, bad_prompt_version2, epiCPhotoGasm-colorfulPhoto-neg, FastNegativeV2, verybadimagenegative_v1.3"
        ...
        for i in range(len(images)):
            with io.BytesIO() as buf:
                ...
                # print("Image size: ", images[i].size)
                # # Upscaling the Image
                ...
                # print("Image size after upscaling: ", images[i].size)
        ...
        try:
            # Delete the input image and the black and white image
            subprocess.run(["rm", input_img_path])
        except:
            pass
...
def main():
    ...
    link = "https://s3-external-1.amazonaws.com/media.twiliocdn.com/ACb1e79ac04da412f3b346abf2330d062d/7e2164f72ecac864ec5339951addca0f"
    if link:
        ...
        # Resize by 0.25x
        # inpu_img = inpu_img.resize((int(width * 0.5), int(height * 0.5)))
    ...
    for i, image in enumerate(images):
        if is_dev:
            a, b, c, d = image
        with open(f"shoe{i}.png", "wb") as f:
            if is_dev:
                f.write(a)
            else:
                f.write(image)
        if is_dev:
            b.save(f"shoe{i}_mask.png")
            c.save(f"shoe{i}_diff.png")
            d.save(f"shoe{i}_orig.png")
