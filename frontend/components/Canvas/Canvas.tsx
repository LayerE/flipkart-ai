...
export default function Canvas() {
  ...
  useEffect(() => {
    ...
    const EditorBoxText = new fabric.Text("Place Your Product Here", {
      left:  20, // center of the rectangle
      top: 20 , // center of the rectangle
      fontSize: 12,
      // originX: "center",
      // originY: "center",
      selectable: false,
    ...
    canvasInstanceRef.on("object:moving", function (event) {
      ...
      if (
        e.layerX >= canvasInstanceRef.left &&
        e.layerX <= canvasInstanceRef.left + canvasInstanceRef.width &&
        e.layerY >= canvasInstanceRef.top &&
        e.layerY <= canvasInstanceRef.top + canvasInstanceRef.height
      ) {
  ...
  return (
    <>
      ...
      <CnavasWrapper className="" ref={outerDivRef}>
